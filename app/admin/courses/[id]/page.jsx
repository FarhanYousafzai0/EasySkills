...
export default function AdminCourseBuilderPage() {
  ...
  return (
<div className="min-h-screen bg-gradient-to-br from-[#9E8CFF]/15 via-white to-[#7866FA]/10 text-slate-800">
  <div className="max-w-7xl mx-auto px-4 py-4 md:py-6 flex flex-col gap-4">
    {/* Top bar */}
    <div className="flex items-center justify-between gap-3 bg-white/80 backdrop-blur-md border border-slate-200 rounded-2xl px-4 py-3 shadow-sm">
      ...
      <div className="flex items-center gap-2 text-[11px]">
        <span className="px-3 py-1 rounded-full bg-gradient-to-r from-[#9E8CFF]/10 to-[#7866FA]/10 border border-[#9E8CFF]/40 text-[#4B3ABF]">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[320px_minmax(0,1fr)] gap-4 md:gap-6">
      {/* LEFT: Sections + items */}
      <aside className="bg-white border border-slate-200 rounded-2xl p-3 md:p-4 shadow-md flex flex-col min-h-[480px]">
        {/* Course header */}
          ...
          {course.thumbnailUrl && (
            <div className="relative">
              <img
                ...
                alt={course.title}
                className="w-12 h-12 md:w-14 md:h-14 rounded-xl object-cover border border-slate-200"
              />
              <span className="absolute -bottom-1 -right-1 bg-gradient-to-r from-[#9E8CFF] to-[#7866FA] text-[9px] px-1.5 py-0.5 rounded-full border border-white text-white shadow-sm">
                {course.level || "Course"}
              </span>
            </div>
          )}
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-1">
            ...
            <div
              ...
              className={`rounded-xl border transition-all duration-150 ${
                activeSectionId === section._id
                  ? "border-[#9E8CFF] bg-[#F5F3FF] shadow-md"
                  : "border-slate-200 bg-slate-50 hover:border-[#9E8CFF]/60 hover:bg-white"
              } overflow-hidden`}
            >
              {/* Section header */}
              <div className="flex items-center justify-between px-3 py-2.5">
                <div className="flex items-center gap-2">
                  <div className="p-1.5 rounded-lg bg-[#9E8CFF]/10 border border-[#9E8CFF]/40">
                    <Layers size={14} className="text-[#4B3ABF]" />
                  </div>
                  <input
                    className="bg-transparent text-xs font-semibold outline-none w-40 md:w-48 text-slate-900 placeholder-slate-400"
                    value={section.title}
                    onChange={(e) => {
              ...
              <div className="bg-white border-t border-slate-100">
                {section.items?.map((item) => (
                  ...
                  >
                    <div className="p-1 rounded-lg bg-white border border-[#9E8CFF]/50 text-[#4B3ABF] shrink-0">
                      <PlayCircle size={14} />
                    </div>
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 text-[11px] text-[#4B3ABF] hover:bg-slate-100 cursor-pointer"
                >
                  <Plus size={13} />
          ...
          {/* New section card */}
          <div className="mt-2 bg-white border border-dashed border-[#9E8CFF]/60 rounded-2xl p-3">
            <p className="text-[11px] text-slate-700 mb-2 font-medium">
              Add new section
            </p>
            <div className="flex gap-2">
              <input
                className="flex-1 bg-white border border-slate-200 rounded-lg px-2.5 py-2 text-xs outline-none focus:border-[#9E8CFF] text-slate-900 placeholder-slate-400"
              ...
              <motion.button
                ...
                className="px-3 py-2 text-xs rounded-lg bg-gradient-to-r from-[#9E8CFF] to-[#7866FA] text-white font-semibold disabled:opacity-60 flex items-center gap-1 shadow-md cursor-pointer"
              >
                {sectionSaving ? (
      ...
      {/* RIGHT: Player + form */}
      <main className="bg-white border border-slate-200 rounded-2xl p-4 md:p-6 shadow-md flex flex-col">
        {/* Player */}
        <div className="w-full aspect-video rounded-2xl border border-slate-200 bg-slate-100 mb-4 overflow-hidden flex items-center justify-center">
          {videoForm.videoUrl ? (
            <video
            ...
            <div className="flex flex-col items-center justify-center text-slate-500 text-sm">
              <div className="p-3 rounded-full bg-white border border-slate-200 mb-2">
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                ...
                className="w-full rounded-lg bg-white border border-slate-200 px-3 py-2 text-xs outline-none focus:border-[#9E8CFF] text-slate-900 placeholder-slate-400"
            ...
            <div>
              ...
              <textarea
                ...
                onChange={handleVideoChange}
                rows={4}
                className="w-full rounded-lg bg-white border border-slate-200 px-3 py-2 text-xs outline-none focus:border-[#9E8CFF] text-slate-900 resize-none placeholder-slate-400"
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                ...
                <motion.label
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="inline-flex items-center gap-2 px-3 py-2 rounded-lg cursor-pointer bg-gradient-to-r from-[#9E8CFF] to-[#7866FA] text-white text-[11px] font-semibold shadow-md"
                >
                  {uploadingVideo ? (
                    <>
                      <Loader2 size={14} className="animate-spin" />
                ...
                {videoForm.videoUrl && (
                  <span className="text-[10px] text-emerald-700 truncate max-w-[150px]">
                    {videoForm.videoUrl}
                  </span>
                )}
              </div>
              <p className="text-[10px] text-slate-500 mt-1">
                Uses Cloudinary unsigned upload preset. Max size depends on your
                Cloudinary plan.
              </p>
            </div>
            ...
            <div className="pt-1">
              <motion.button
                ...
                className="w-full rounded-xl bg-gradient-to-r from-[#9E8CFF] to-[#7866FA] text-white text-xs font-semibold py-2.5 flex items-center justify-center gap-2 disabled:opacity-60 shadow-lg cursor-pointer"
              >
                {savingVideo ? (
                  <>
